...
def ClusterMap(clusters, vals, indicesToShow,
    ...
    black_offset = axis.transData + transforms.ScaledTranslation(dx, dy, axis.figure.dpi_scale_trans)
    ...
    # Note: this can be an over-estimate many clusters might be outside the bounding box.
    elementCount = (2*len(indicesToShow)) + (radarBG_alpha > 0.0) + (dimmerBox_alpha > 0.0)
    ...
    for index, goodMembers in enumerate(clustMembers) :
        ...
        outline = axis.contour(lons, lats, clustMask,
                               [0, 1], colors='k', linewidths=2.5, zorder = zorders[zorderIndex + 0])
        SetContourZorder(outline, zorders[zorderIndex + 1])
        SetContourTransform(outline, black_offset)

        # white highlight... save time by re-using the contour data.
        ...
        MakeReflectPPI(vals, lats, lons, mask=clustMask, ax=axis, zorder=zorders[zorderIndex + 1],
                       axis_labels=False, colorbar=False, **kwargs)
        zorderIndex += 2
